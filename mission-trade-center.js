...
(function () {
  ...
  const lots = [
    ...
    { number: 1,  cluster: 'B', acres: 3.57, sqft: 155433, status: 'available', adjacentLots: [2],          notes: '' },
    { number: 2,  cluster: 'B', acres: 3.12, sqft: 135792, status: 'available', adjacentLots: [1, 3],       notes: 'Smallest lot in inventory' },
    { number: 3,  cluster: 'B', acres: 4.64, sqft: 202243, status: 'available', adjacentLots: [2, 4],       notes: 'Largest lot in Cluster B' },
    { number: 4,  cluster: 'B', acres: 3.51, sqft: 153058, status: 'available', adjacentLots: [3, 5, 8],    notes: '' },
    { number: 5,  cluster: 'B', acres: 3.37, sqft: 146894, status: 'available', adjacentLots: [4, 6],       notes: '' },
    { number: 6,  cluster: 'B', acres: 3.70, sqft: 161024, status: 'available', adjacentLots: [5, 8, 9],    notes: '' },
    { number: 7,  cluster: 'B', acres: 3.93, sqft: 171367, status: 'available', adjacentLots: [9],          notes: '' },
    { number: 8,  cluster: 'B', acres: 3.70, sqft: 161268, status: 'available', adjacentLots: [4, 6, 9],    notes: '' },
    { number: 9,  cluster: 'B', acres: 3.99, sqft: 173839, status: 'available', adjacentLots: [6, 7, 8],    notes: '' },
    // Cluster A — Northeast Industrial Block (Lots 10–16)
    { number: 10, cluster: 'A', acres: 5.29, sqft: 230638, status: 'available', adjacentLots: [11, 15, 16], notes: '' },
    ...
    { number: 16, cluster: 'A', acres: 8.39, sqft: 365620, status: 'available', adjacentLots: [13, 14, 15], notes: 'Cold storage anchor — Conway Ave frontage' }
  ];
  ...
  function updateSelectionBar() {
    ...
    if (inquireBtn) inquireBtn.addEventListener('click', () => {
      autoFillInquiryLots(lotNums);
      // Route through the tab system so we switch to Resources & Contact first.
  ...
  /* =====================================================================
     MASTER PLAN SVG INTERACTIVITY
     Highlight available lots; show MEDA parcels muted with overlay.
     ===================================================================== */
  ...
  function positionTooltip(tt, el) {
    const wrap = document.getElementById('master-plan');
    if (!wrap) return;
  ...
  function autoFillInquiryLots(nums) {
    ...
    Array.from(sel.options).forEach(opt => {
      opt.selected = nums.includes(parseInt(opt.value, 10));
    });
  }
