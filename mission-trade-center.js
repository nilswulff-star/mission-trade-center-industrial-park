...
(function () {
  ...
  /* =====================================================================
     INQUIRY FORM — lots-of-interest chip UI
     The visible chip container shows what's selected; a hidden input
     (#inq-lots) holds the comma-separated tokens for form submit. The
     dropdown (#inq-lots-add) is for manual additions; programmatic
     additions come from the inventory section's "Inquire on selected"
     and per-lot "Inquire on Lot N" actions.
     ===================================================================== */
  const inqLotsState = new Set(); // tokens: "1".."16", "entire-park", "cluster-A", etc.

  // Pretty label for a token — matches the dropdown options
  function inqLotLabel(token) {
    if (token === 'entire-park') return 'Entire park';
    if (token === 'multiple-tbd') return 'Multiple lots — TBD';
    if (token === 'cluster-A') return 'All Cluster A (10–16)';
    if (token === 'cluster-B') return 'All Cluster B (1–9)';
    const n = parseInt(token, 10);
    if (!isNaN(n)) {
      const lot = lots.find(l => l.number === n);
      if (lot) return `Lot ${n} · ${lot.acres.toFixed(2)} ac`;
      return `Lot ${n}`;
    }
    return token;
  }

  function renderInqLotChips() {
    const container = document.getElementById('inq-lots-chips');
    const empty = document.getElementById('inq-lots-empty');
    const hidden = document.getElementById('inq-lots');
    if (!container || !hidden) return;

    // Sort tokens: numeric first ascending, then special tokens in fixed order
    const tokens = Array.from(inqLotsState);
    const specialOrder = ['entire-park', 'cluster-A', 'cluster-B', 'multiple-tbd'];
    tokens.sort((a, b) => {
    ...
    // Clear and rebuild
    Array.from(container.querySelectorAll('.inq-chip')).forEach(c => c.remove());
    if (tokens.length === 0) {
      ...
      tokens.forEach(token => {
        ...
        chip.className = 'inq-chip';
        chip.dataset.token = token;
        chip.innerHTML =
          '<span class="inq-chip__label"></span>' +
          '<button type="button" class="inq-chip__remove" aria-label="Remove">&times;</button>';
        chip.querySelector('.inq-chip__label').textContent = inqLotLabel(token);
        chip.querySelector('.inq-chip__remove').addEventListener('click', () => {
          inqLotsState.delete(token);
          renderInqLotChips();
        });
  ...
  function autoFillInquiryLots(nums) {
    // Replace the current selection with the given lot numbers.
    inqLotsState.clear();
    nums.forEach(n => inqLotsState.add(String(n)));
    // Defer so the tab-switch (if any) has time to make the form visible
    const apply = () => {
      renderInqLotChips();
      const container = document.getElementById('inq-lots-chips');
      if (container) {
        container.classList.add('inq-lots--flash');
        setTimeout(() => container.classList.remove('inq-lots--flash'), 1200);
      }
    };
    apply();
  ...
  function bindInqLotsPicker() {
    const picker = document.getElementById('inq-lots-add');
    if (!picker) return;
  ...
  function bindForm() {
    ...
    form.addEventListener('submit', (e) => {
      ...
      const lotsSel = Array.from(inqLotsState);
      const payload = {
        name: fd.get('name'),
        firm: fd.get('firm'),
        email: fd.get('email'),
  ...
  /* =====================================================================
     GALLERY (no-op — placeholder system removed in v1.8 once real
     imagery was placed. Function retained as a defensive stub so
     init() doesn't break if called.)
     ===================================================================== */
  function bindGallery() {
    // No-op. All gallery images are now real files.
  }

  /* =====================================================================
     LIGHTBOX
     Click any image marked [data-lightbox] to open it full-screen at
     native resolution. Prev/Next cycles through all lightbox images on
     the page (sorted by document order). Esc / click overlay / click
     image / × button closes. Arrow keys navigate. Body scroll locked
     while open.
     ===================================================================== */
  function bindLightbox() {
    ...
    function refreshItems() {
      // Re-scan the DOM for clickable images each time, since some live in
      // hidden tab panels and their order may change as the page evolves.
      items = $$('img[data-lightbox]');
    }

    function captionFor(imgNode) {
      // Prefer the parent <figure>'s <figcaption>; fall back to alt text.
      const fig = imgNode.closest('figure');
      if (fig) {
    ...
    function show(idx) {
      ...
      // Reset stage scroll so user starts at top-left of large images
      stage.scrollTo({ top: 0, left: 0, behavior: 'instant' in stage ? 'instant' : 'auto' });
    }
    ...
    function close() {
      ...
      // Clear src so memory drops the big image when not in view
    ...
    // Delegate clicks on any [data-lightbox] image
    document.addEventListener('click', (e) => {
    ...
    overlay.addEventListener('click', (e) => {
      // Close on click of overlay or image, but not nav/caption/close
      if (e.target === overlay || e.target === imgEl || e.target === stage) {
        close();
      }
    });
    ...
    // Touch swipe (horizontal) for prev/next
    ...
    stage.addEventListener('touchend', (e) => {
      ...
      touchStartX = null;
      // Threshold of 60px so accidental taps don't trigger
      if (Math.abs(dx) > 60) {
        if (dx < 0) next();
        else prev();
      }
